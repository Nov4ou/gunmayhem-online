...
Room settings select one of the original custom-game implementations before root frame 10 starts: mode 1 is Last Man Standing and mode 4 is Gun Game. Last Man Standing uses the configured lives value. Gun Game retains unlimited respawns, no grenades, crates, or power-ups, one progression step per elimination, ammunition regression, and victory after completing level 16. Its original 15-weapon list is applied in reverse order so the available weapon strength decreases as a player approaches victory. The networking layer synchronizes inputs for both modes without recreating either ruleset.
