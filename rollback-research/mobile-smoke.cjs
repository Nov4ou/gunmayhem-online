...
(async()=>{
  ...
  try{
    ...
    for(const page of pages){const frame=page.frames().find(candidate=>candidate.url().includes('runtime.html'));assert(frame);if(gameMode==='gun-game')await frame.waitForFunction(()=>call('netState').players.slice(0,2).every(Boolean));const native=await frame.evaluate(()=>call('netState'));assert.equal(native.mode,gameMode==='gun-game'?4:1);assert.deepEqual(native.profiles.slice(0,2).map(({color,shirt,hat})=>({color,shirt,hat})),expectedProfiles);if(gameMode==='gun-game')assert(native.players.slice(0,2).every(player=>player.currentlevel===1&&player.currentgun===44));}
